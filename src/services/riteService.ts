...
const userCollectionRef = collection(db, "rite");

const getAllRites = async () => {
  try {
...
const updateRite = async (id: any, updatedRite: any) => {
  try {
    const userDoc = doc(userCollectionRef, id);
    await updateDoc(userDoc, updatedRite);
...
export { getAllRites, addRite, updateRite, deleteRite };
